  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 md:p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-3 md:mb-4">Recent Activity</h3>
            ...
            >
              <div className="flex items-start sm:items-center space-x-3 md:space-x-4">
                <div className={`p-2 rounded-lg shrink-0 ${
                  activity.type === 'booking' ? 'bg-purple-100 text-purple-600' : 'bg-pink-100 text-pink-600'
                }`}>
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-gray-800 text-sm md:text-base truncate">{activity.title}</p>
                  <p className="text-xs md:text-sm text-gray-600">{activity.date}</p>
                </div>
              </div>
              <span className={`px-2.5 py-1 rounded-full text-xs md:text-sm ${activity.statusColor} self-start sm:self-center shrink-0`}>
                {activity.status}
              </span>
            </div>
          ))}
